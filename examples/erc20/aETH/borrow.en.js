//<![CDATA[
class Token {

    constructor(token, card) {
        this.token = token;
        this.card = card;
    }

    render() {
        const decimals18 = 1e+18;
        let variableAPR = ((this.token.variableInterestRateBorrow / 1e+27) * 100).toFixed(2);
        let collateralValueInEth = (this.token.totalCollateral / decimals18).toFixed(2);
        let daiPriceInEth = (this.token.assetPriceDAI / decimals18).toFixed(2);
        let borrowable = (this.token.availableBorrowsETH / decimals18).toFixed(2);
        let borrowed = ((this.token.totalBorrows / decimals18) / daiPriceInEth).toFixed(2);

        return `
        <div class="borrow action">
           <h1>Borrow</h1>
           <h2><img src="data:image/png;base64,"></h2>
           <input id="redeemAmount" type="number" placeholder="0">
           <p class="hint">Available to borrow: ${borrowable / daiPriceInEth} DAI</p>
           <table>
              <caption>Details</caption>
              <tbody>
                 <tr>
                    <th>Variable Borrow APR </th>
                    <td>${variableAPR}%</td>
                 </tr>
                 <tr>
                    <th>Maximum you can borrow</th>
                    <td>${borrowable} ETH (${borrowable / daiPriceInEth} DAI)</td>
                 </tr>
                 <tr>
                    <th>You Already Borrowed</th>
                    <td>${borrowed} DAI</td>
                 </tr>
                 <tr>
                    <th>New Health Factor</th>
                    <td>${this.token.healthFactor.toFixed(2)}</td>
                 </tr>
                 <tr>
                    <th>Your collateral is worth</th>
                    <td>${collateralValueInEth} ETH (${collateralValueInEth / daiPriceInEth} DAI)</td>
                 </tr>
              </tbody>
           </table>
        </div>`;
    }
}

web3.tokens.dataChanged = (oldTokens, updated, tokenIdCard) => {
    document.getElementById(tokenIdCard).innerHTML = new Token(updated.token, updated.card).render();
};
//]]>
